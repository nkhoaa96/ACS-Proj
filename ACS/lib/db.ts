...
export let tasksCollection: Collection<MongoTypes.Task>,
  ...
  logsCollection: Collection<MongoTypes.Log>;
...
onConnect(async (db) => {
  ...
  logsCollection = db.collection('config');
});
...
export function getLogs(): Promise<Record<string, any>[]> {
