  ...
  if (task != null) {
    if (task.status === 'done') {
      ...
      const data= {
        ...
        device: device,
        kind: kind,
        serialNumber: device["DeviceID.SerialNumber"].value[0],
        productClass: device["DeviceID.ProductClass"].value[0],
        pppoeUser: device["VirtualParameters.PPPoEACName"].value[0],
        type: type,
        account: account,
        resource: resource,
