...
export function processLogsFilter(filter: Expression): Expression {
  return map(filter, (exp) => {
    if (!isArray(exp)) return exp;

    if (exp[0] === 'PARAM') {
      const p = exp[1];
      if (p === 'kind') return ['PARAM', 'kind'];
      else if (p === 'action') return ['PARAM', 'action'];
      else if (p === 'serialNumber') return ['PARAM', 'serialNumber'];
      else if (p === 'productClass') return ['PARAM', 'productClass'];
      else if (p === 'type') return ['PARAM', 'type'];
      else if (p === 'account') return ['PARAM', 'account'];
      else if (p === 'resource') return ['PARAM', 'resource'];
      else if (p === 'time') return ['PARAM', 'time'];
    } else if (
      isArray(exp[1]) &&
      exp[1][0] === 'PARAM' &&
      exp[1][1].startsWith('Tags.')
...
export function processDeviceSort(
  ...
  for (const [k, v] of Object.entries(sort)) {
    ...
    else if (k === 'VirtualParameters.PPPOEACNAME') s['VirtualParameters'] = v;
    else s[`${k}._value`] = v;
  }
      ...
      if (!root) {
        if (name === '_lastInform') {
          ...
          for (const t of tree as string[]) {
            const encodedTagVal = encodeTag(t);
            encodedTagVals.push(encodedTagVal);
            output[`Tags.${encodedTagVal}`] = {
          ...
          output['TagValues'] = {
            value: [encodedTagVals.join(','), 'xsd:string'],
            valueTimestamp: timestamp,
            writable: false,
            writableTimestamp: timestamp,
            object: false,
